...
| --- | --- | --- | --- |
| `Database::open` | `metadata_path` | `Database` | Open or initialize metadata, taxonomy, and default photo storage. |
...
| `Database::register_photo_library` | `root_path: &Path`, `database_path: &Path`, `display_name: Option<&str>` | `CoreResult<PhotoLibraryRegistration>` | Register an existing or new photo library DB and activate it. |
| `Database::switch_photo_library` | `library_uuid: &str` | `CoreResult<PhotoLibraryRegistration>` | Activate a registered library without changing other libraries. |
...
| `Database::relocate_photo_library_database` | `library_uuid: &str`, `destination: &Path` | `CoreResult<PhotoLibraryRegistration>` | Move one library DB and update metadata. |
| `Database::relocate_taxonomy_database` | `destination: &Path` | `CoreResult<DatabaseLocations>` | Move the taxonomy DB and update metadata. |
